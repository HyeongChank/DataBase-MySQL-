...
-- 다시
select 수강신청번호, 과목번호, 평점, 취득일자(select 평점 as '취득' from 수강신청내역 where 평점 in (4,3,2,1)) '취득여부' from 수강신청내역;
...
-- 다시
select 학번, 이름, 학과 ,학과명(select 학과명 as '타과' from 학과 where 학과명 != 컴퓨터정보학과) from 학생, 학과 where 학생.학과 = 학과.학과번호;
...
select 수강신청번호, 수강신청.학번, 학생.이름 from 수강신청, 학생 where 수강신청.학번=학생.학번 and left(수강신청번호,2)='18';

-- 덜 푼 문제는 보완하여 다시 제출하겠습니다 감사합니다
